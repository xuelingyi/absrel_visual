...
pdf(paste0(path, "/", t, "_absrel.pdf"), width = 9)
if(min(T1$P_corrected) <= 0.05){
  ...
  if(as.numeric(data$analysis$version) >= 2.5){
    ...
    ali.ER2.dna = data.frame(label = names(ali))
    ...
    for(s in sort(unique(T3[T3$ER>2, "site"]))){
      ...
      ali.ER2.dna$one = sapply(ali.ER2.dna$label, FUN=function(x){as.character(ali[[x]][3*(s-1)+1])})
      ali.ER2.dna$two = sapply(ali.ER2.dna$label, FUN=function(x){as.character(ali[[x]][3*(s-1)+2])})
      ali.ER2.dna$three = sapply(ali.ER2.dna$label, FUN=function(x){as.character(ali[[x]][3*s])})
      names(ali.ER2.dna)[(ncol(ali.ER2.dna)-2):ncol(ali.ER2.dna)] = paste0("aa", s, "_", c(1:3))
      ...
      names(ali.ER2)[c(ncol(ali.ER2)-1, ncol(ali.ER2))] = c(paste0("site", s, "ER_", round(max(T3[T3$site ==s & T3$branches %in% T1[T1$P_corrected<=0.05, "branches"], "ER"])), "_"), 
                                                            paste0(s, "_ER_", round(max(T3[T3$site ==s & T3$branches %in% T1[T1$P_corrected<=0.05, "branches"], "ER"])), "_"))
    }
